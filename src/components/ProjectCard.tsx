...
const ProjectCard: React.FC<ProjectCardProps> = ({ title, description, skills, githubLink, projectLink, imageUrl, types}) => {
    ...
    <div className="bg-grey-4 text-white p-4 rounded-lg border border-grey-3 h-full">
        <div className="relative">
            <img src={imageUrl} alt={`${title}`} className="w-full h-[250px] object-cover mb-4 rounded-lg" />
            <div className="absolute top-2 right-2 space-y-2">
                {githubLink && (
                <a href={githubLink} target="_blank" rel="noopener noreferrer" className=" bg-white h-[45px] w-[45px] rounded-full flex items-center justify-center ">
                    <GitHubIcon fontSize="large" className="text-grey-1" />
                </a>)}
                {projectLink && (
                <a href={projectLink} target="_blank" rel="noopener noreferrer" className=" bg-white h-[45px] w-[45px] rounded-full flex items-center justify-center ">
